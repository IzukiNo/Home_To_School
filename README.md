...
| **Graph Modes**     | **NEW!** Directed/Undirected toggle with auto-detection           |
...
### 🔄 Graph Direction Toggle (NEW!)

- **Directed/Undirected Modes**: Switch between directed and undirected graph representations
- **Auto-Detection**: Automatically detects graph type when importing (symmetric = undirected)
- **Smart Conversion**: Seamlessly converts between modes while preserving graph structure
- **State Management**: Saves directed state when switching to undirected mode
- **Visual Feedback**: Toggle button shows current mode with clear icons and colors
- **Documentation**: See [GRAPH_DIRECTION_TOGGLE.md](docs/GRAPH_DIRECTION_TOGGLE.md) for complete details

### 📊 Import/Export

- **Adjacency Matrix Export**: Export your graph as an adjacency matrix
- **Matrix Import**: Import graphs from adjacency matrix format
- **Smart Import**: Auto-detects directed vs undirected graphs from matrix symmetry
...
const numConnections = Math.random() < 0.4 ? 2 : Math.random() < 0.8 ? 3 : 1; // 40%/40%/20%
